...
// CVOPIntYUVBA
Void getBlockDataFromMB (const CVOPIntYUVBA* pvopfMB, CIntImage*& pfiBlk, CIntImage*& pfiB, BlockNum blkNum)
{	
	if (blkNum == ALL_Y_BLOCKS) {
		...
		pfiB = new CIntImage (*pvopfMB -> getPlane (BY_PLANE));
		pfiBlk = new CIntImage (*pvopfMB -> getPlane (A_PLANE));
		...
		CoordI top = rct.top + idy * BLOCK_SIZE;
		CRct rctF (left, top, left + BLOCK_SIZE, top + BLOCK_SIZE);
		pfiBlk = new CIntImage (*pvopfMB -> getPlane (A_PLANE), rctF);
		pfiB = new CIntImage (*pvopfMB -> getPlane (BY_PLANE), rctF);
	}
}

Void getTextureDataFromMB (const CVOPIntYUVBA* pvopfMB, CIntImage*& pfiBlk, BlockNum blkNum)
{	
	if (blkNum == ALL_Y_BLOCKS) {
		pfiBlk = new CIntImage (*pvopfMB -> getPlane (Y_PLANE));
...
Void getBinaryDataFromMB (const CVOPIntYUVBA* pvopfMB, CIntImage*& pfiA, BlockNum blkNum)
{	
