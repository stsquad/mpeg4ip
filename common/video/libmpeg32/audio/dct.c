...
static void swap_cmplx(mpeg3_complex_t *a, mpeg3_complex_t *b)
{
	mpeg3_complex_t tmp;
		...
		int render)
{
	...
	if(render)
	{
		for(i = 0; i < bsi->nfchans + bsi->lfeon; i++)
		{
			if(i < 5 && audblk->blksw[i])
	...
	audio->pcm_point += AC3_N / 2 * (bsi->nfchans + bsi->lfeon);
