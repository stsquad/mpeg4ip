...
static mpeg3audio_t* allocate_struct(mpeg3_t *file, mpeg3_atrack_t *track)
{
...
int mpeg3audio_replace_buffer(mpeg3audio_t *audio, long new_allocation)
{
	...
	if(!audio->pcm_sample)
	{
		audio->pcm_sample = malloc(sizeof(float) * new_allocation * audio->channels);
		audio->pcm_allocated = new_allocation;
	}
	else
	{
		float *new_samples = malloc(sizeof(float) * new_allocation * audio->channels);
...
{
	mpeg3audio_t *audio = allocate_struct(file, track);
	int result = 0;
