...
static int get_scale_factors_1(mpeg3audio_t *audio,
		int *scf, 
...
static int get_scale_factors_2(mpeg3audio_t *audio,
		int *scf,
		struct gr_info_s *gr_info,
		int i_stereo)
{
...
static int dequantize_sample(mpeg3audio_t *audio,
		...
		int part2bits)
{
	...
	int32_t mask = mpeg3bits_getbits(audio->astream, num);
		...
		for(i = 0; i < 2; i++) 
		{
			...
			for( ; lp; lp--, mc--) 
			{
        		...
        		if(x == 15 && h->linbits) 
				{
        			max[lwin] = cb;
        			REFRESH_MASK;
        			x += ((u_int32_t)mask) >> (BITSHIFT + 8 - h->linbits);
        			num -= h->linbits + 1;
        			mask <<= h->linbits;
        			if(mask < 0)
        		...
        		if(y == 15 && h->linbits) 
				{
        			max[lwin] = cb;
        			REFRESH_MASK;
        			y += ((u_int32_t) mask) >> (BITSHIFT + 8 - h->linbits);
        			num -= h->linbits + 1;
        			mask <<= h->linbits;
        			if(mask < 0)
...
//printf("III_dequantize_sample 3 %d %04x\n", audio->astream->bit_number, mpeg3bits_showbits(audio->astream, 16));
	...
	if(part2remain < 0) 
	{
      	fprintf(stderr,"dequantize_sample: Can't rewind stream %d bits!\n", -part2remain);
      	return 1; /* -> error */
	}
	return 0;
}

static int get_side_info(mpeg3audio_t *audio,
...
static int hybrid(mpeg3audio_t *audio,
...
static int antialias(mpeg3audio_t *audio,
		float xr[SBLIMIT][SSLIMIT],
		struct gr_info_s *gr_info)
{
	int sblim;
 ...
 */
static int calc_i_stereo(mpeg3audio_t *audio, 
...
int mpeg3audio_dolayer3(mpeg3audio_t *audio, int render)
{
	int gr, ch, ss;
...
/* Stereo */
  	...
  	else 
	{
  	...
  	if(get_side_info(audio, &sideinfo, channels, ms_stereo, sfreq, single, audio->lsf))
		return 1;
...
//printf("dolayer3 4 %04x\n", mpeg3bits_showbits(audio->astream, 16));
			...
			if(dequantize_sample(audio, hybridIn[0], scalefacs[0], gr_info, sfreq, part2bits))
			{
...
//printf("dolayer3 5 %04x\n", mpeg3bits_showbits(audio->astream, 16));
      	...
      	if(channels == 2) 
		{
    		...
    		if(dequantize_sample(audio, hybridIn[1], scalefacs[1], gr_info, sfreq, part2bits))
			{
				mpeg3bits_use_demuxer(audio->astream);
        		return 1;
			}
