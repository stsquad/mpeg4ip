...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_dx5audio.c,v 1.1 2001/02/05 20:26:27 cahighlander Exp $";
#endif
...
AudioBootStrap DSOUND_bootstrap = {
	"dsound", Audio_Available, Audio_CreateDevice
};
...
void DX5_ThreadInit(_THIS)
{
	SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_HIGHEST);
}

void DX5_WaitAudio_BusyWait(_THIS)
{
	DWORD status;
	DWORD cursor, junk;
	HRESULT result;
...
void DX6_WaitAudio_EventWait(_THIS)
{
	DWORD status;
	HRESULT result;
...
int DX5_OpenAudio(_THIS, SDL_AudioSpec *spec)
{
	HRESULT      result;
