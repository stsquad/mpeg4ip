...
static size_t audio_freq_index = 2;

const char *get_audio_frequency (void)
{
  return (audio_freq[audio_freq_index]);
}

const char **get_audio_frequencies (size_t &max, size_t &current_index)
{
  max = 8;
  current_index = audio_freq_index;
  return audio_freq;
}

int set_audio_parameters (size_t codec_index,
			  size_t freq_index,
			  size_t bitrate,
			  const char **errmsg)
{
  if (bitrate < 8) {
    *errmsg = "Bitrate too small";
    return (0);
  }
  audio_codec_index = codec_index;
  audio_freq_index = freq_index;
  audio_kbps = bitrate;
  return (1);
}
