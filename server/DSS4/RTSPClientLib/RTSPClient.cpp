...
OS_Error RTSPClient::SendUDPSetup(UInt32 inTrackID, UInt16 inClientPort)
{
	...
	if (!fTransactionStarted)
	{
		...
		if (fTransportMode == kPushMode)
			...
			::sprintf(fSendBuffer, "SETUP %s/trackID=%lu RTSP/1.0\r\nCSeq: %lu\r\n%sTransport: RTP/AVP;unicast;client_port=%u-%u\r\n%sUser-agent: %s\r\n\r\n", fURL.Ptr, inTrackID, fCSeq, fSessionID.Ptr, inClientPort, inClientPort + 1, fSetupHeaders, fUserAgent);
	}
	return this->DoTransaction();
}
...
OS_Error RTSPClient::SendTCPSetup(UInt32 inTrackID)
{
	...
	{	
		...
		if (fTransportMode == kPushMode)
			::sprintf(fSendBuffer, "SETUP %s/trackID=%lu RTSP/1.0\r\nCSeq: %lu\r\n%sTransport: RTP/AVP/TCP;unicast;mode=receive\r\nUser-agent: %s\r\n\r\n", fURL.Ptr, inTrackID, fCSeq, fSessionID.Ptr, fUserAgent);
		else 
			::sprintf(fSendBuffer, "SETUP %s/trackID=%lu RTSP/1.0\r\nCSeq: %lu\r\n%sTransport: RTP/AVP/TCP;unicast\r\n%sUser-agent: %s\r\n\r\n", fURL.Ptr, inTrackID, fCSeq, fSessionID.Ptr, fSetupHeaders, fUserAgent);
	}
