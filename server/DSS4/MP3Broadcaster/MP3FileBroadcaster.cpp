...
int MP3FileBroadcaster::PlaySong(char *fileName, char *currentFile, bool preflight, bool fastpreflight)
{
	...
	if (strlen(mArtist) != 0)
		::sprintf(mSong, "%s - %s", mTitle, mArtist);
	else
		::strcpy(mSong, mTitle);
	...
	::printf("Bitrate = %dkbits, frequency = %dKHz\n", mBitRate, mFrequency/1000);
	...
	if (mDesiredFrequency == -1)
	{
		...
		printf("Setting required frequency to %dKHz\n", mFrequency/1000);
	}
	...
	UInt64 startTime = OS::Milliseconds();
	int totalBytes = 0;
	int numFrames = 0;	// amount of play time this buffer represents
	int leftOver = 0;	// we may have a partial buffer left over from last read
	UInt64 properElapsedTime;
	while(true)
	{
		if (!preflight)
		{
			...
			UInt64 nextSendTime = mBroadcastStartTime + properElapsedTime;

			UInt64 currentTime = OS::Milliseconds();
			if (nextSendTime > currentTime)
				OSThread::Sleep(nextSendTime - currentTime);
		}
...
//	::printf("Sent %d bytes in %qd milliseconds = %qd bits per second\n", totalBytes, elapsed, rate);
...
bool MP3FileBroadcaster::ReadV2_2Tags()
{
	...
	while (ptr-buffer < totalTagLen)
	{
		...
		if ((strlen(mTitle) > 0) && (strlen(mArtist) > 0))
