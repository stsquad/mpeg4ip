...
static char*			sBroadcastPassword 		= " ";
...
static char*	sLogName		= "mp3_access";
static char*	sLogDir		 	= "/Library/QuickTimeStreaming/Logs/";
...
{
	return (::strcmp(fMountpoint, mp) == 0);
}
...
QTSS_Error MP3BroadcasterSession::GetBroadcastHeaders()
{
	...
	else
	{
		...
		DTRACE1("MP3BroadcasterSession::GetBroadcastHeaders() - read failed with err = %ld\n", theErr);
		Assert(0);
	}
	return theErr;
}
...
QTSS_Error MP3BroadcasterSession::GetBroadcastData()
{
	...
	{
		DTRACE1("MP3BroadcasterSession::GetBroadcastData() - read failed with err = %ld\n", theErr);
		Assert(0);
		ShutDownState();
	}
	return theErr;
...
QTSS_Error MP3ClientSession::SendResponse()
{
	...
	if (GetState() == MP3ClientSession::kClientInitState)
	{
		...
		theErr = QTSS_Write(GetStreamRef(), kClientAcceptHeader, ::strlen(kClientAcceptHeader), NULL, qtssWriteFlagsBufferData);		
		if (theErr != QTSS_NoErr)
		{
			SetState(MP3ClientSession::kClientShutDownState);
			return theErr;
...
QTSS_Error MP3ClientSession::SendMP3Data(char* buffer, UInt32 bufferlen)
{
	...
	if (theErr == QTSS_WouldBlock)
	{
		...
		QTSS_RequestEvent(GetStreamRef(), QTSS_WriteableEvent);
                DTRACE2("Got blocked at time %qd, numRetries = %ld\n", curTime, fRetryCount);
		if (fBlockTime == 0)
			fBlockTime = curTime;
		fRetryCount++;
		...
		{
			SetResult(453);
                        DTRACE1("MP3ClientSession::SendMP3Data - too many tries. Terminating client session = %ld\n", GetSessionID());
			SetState(MP3ClientSession::kClientShutDownState);
		}
	}
	else if (theErr != QTSS_NoErr)
	{
                DTRACE1("MP3ClientSession::SendMP3Data - Terminating client session = %ld\n", GetSessionID());
		SetState(MP3ClientSession::kClientShutDownState);
	}
	else
	{
...
{	
	...
	if (updateTime > 0) // leave Bit Rate the same if updateTime is 0 also don't divide by 0.
		fCurrentBitRate = bitsInInterval / updateTime;
...
{	
	...
	if (err == QTSS_NoErr)
	{
		...
		if (::strstr(fRequestBuffer, "icy-metadata:1") != NULL)
		{
			fWantsMetaData = true;
		}
		else if (::strstr(fRequestBuffer, "icy-metadata: 1") != NULL)
		{
			fWantsMetaData = true;
		}
                else if (::strstr(fRequestBuffer, "Icy-MetaData:1") != NULL)
...
QTSS_Error MP3BroadcasterQueue::CreateBroadcaster(QTSS_RTSPSessionObject sess, QTSS_StreamRef stream, StrPtrLen& mountpt)
{
	...
	if (elem != 0)
	{
		OSMutexLocker locker(&fMutex);
		fQueue.EnQueue(elem);
		DTRACE1("MP3BroadcasterQueue::CreateBroadcaster() Session(%ld) broadcaster added!\n", uSessID);
...
QTSS_Error MP3BroadcasterQueue::RemoveBroadcaster(QTSS_RTSPSessionObject sess)
{
	...
	while ((elem = iter.GetCurrent()) != 0)
	{
		...
		if ((current != 0) && (current->GetSession() == sess))
		{
			...
			DTRACE1("MP3BroadcasterQueue::RemoveBroadcaster() Session(%ld) broadcaster removed!\n", uSessID);
			delete elem;
			delete current;
			return QTSS_NoErr;
		}
										...
										MP3BroadcasterSession* owner)
{
	...
	if (elem != 0)
	{
		OSMutexLocker locker(&fMutex);
		fQueue.EnQueue(elem);
		DTRACE1("MP3ClientQueue::AddClient() Session(%ld) client added!\n", uSessID);
		// send the "OK" back to the broadcaster
...
QTSS_Error MP3ClientQueue::RemoveClient(QTSS_RTSPSessionObject sess)
{
	...
	while ((elem = iter.GetCurrent()) != 0)
	{
		...
		if ((curClient != 0) && (curClient->GetSession() == sess))
		{
			...
			DTRACE1("MP3ClientQueue::RemoveClient() Session(%ld) client removed!\n", uSessID);
			delete elem;
			delete curClient;
			return QTSS_NoErr;
		}
...
void IncrementMP3SessionCount()
{
	...
	{
		DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrCurConn failed with err = %ld\n", (SInt32)err);
	}
	...
	{
		DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrTotalConn failed with err = %ld\n", (SInt32)err);
	}
}
...
void IncrementTotalMP3Bytes(UInt32 bytes)
{
	...
	{
		DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrTotalBytes failed with err = %ld\n", (SInt32)err);
	}
}
...
Bool16 CheckBandwidth(SInt32 bandwidth)
{
	...
	{
		DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrCurBandwidth failed with err = %ld\n", (SInt32)err);
	}
	return false;
}
...
// request.
inline Bool16 IsHTTPGet(StrPtrLen& theRequest)
{
	StrPtrLen token = theRequest;
	token.Len = 3;
...
{
	Bool16 needsit = false;
	char* tmp = theRequest.GetAsCString();
	
	if (tmp == NULL)
		return false;
	else if (::strstr((const char*)tmp, "icy-metadata:1") != NULL)
		needsit = true;
	else if (::strstr((const char*)tmp, "icy-metadata: 1") != NULL)
		needsit = true;
	else if (::strstr((const char*)tmp, "Icy-MetaData: 1") != NULL)
...
QTSS_Error LogRequest(QTSS_RTSPSessionObject inRTSPSession, MP3ClientSession* client)
{
	...
	::sprintf(logbuffer, "%s \"%s\" [%s] \"%s\" %d %ld %ld\n",
		remoteAddress,
                userAgent,
		theDateBuffer,
		reqBuffer,
...
QTSS_Error FilterRequest(QTSS_Filter_Params* inParams)
{
	...
	UInt32 uSessID = 0L;
	uSessID = GetRTSPSessionID(inParams->inRTSPSession);
	DTRACE1("###Filtering SessionID = %ld\n", uSessID);
#endif
	
	// See if we have exceeded our maximum number of client connections
	if (MP3Session::GetTotalNumMP3Sessions() >= sMaximumConnections)
	{
		// we are currently at our connection limit. just ignore this request.
		return QTSSModuleUtils::SendErrorResponse(theRequest, qtssClientNotEnoughBandwidth,
                                    qtssMsgTooManyClients);
	}
	else if (!CheckBandwidth(kBandwidthToAddEstimate))
	{
		// we are currently at our bandwidth limit. just ignore this request.
		return QTSSModuleUtils::SendErrorResponse(theRequest, qtssClientNotEnoughBandwidth,
                                    qtssMsgTooManyClients);
	}
	
	StrPtrLen mountpoint;
	if (IsBroadcastPassword(theFullRequest) && CheckPassword(inParams, theFullRequest, mountpoint))
	{
	...
	else if ( IsHTTPGet(theFullRequest) )
	{
		...
		// if this was a meta-data transfer URL handle it here
		...
		if (xs == NULL)
		{
			// We didn't find this URL mountpoint. Let some other module handle
			// this URL.
			return QTSS_NoErr;
		}
