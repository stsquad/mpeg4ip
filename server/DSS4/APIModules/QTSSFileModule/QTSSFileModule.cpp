...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params* inParamBlock)
{
	if (isSDP(inParamBlock))
	{
		...
		return QTSSModuleUtils::SendErrorResponse(inParamBlock->inRTSPRequest, qtssClientNotFound, sNoSDPFileFoundErr,&pathStr);
	}
...
// -------- version header 
		...
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sVersionHeader);
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);
...
// -------- owner header
		...
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &ownerStr);		
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);
		
// -------- session header

		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sSessionNameHeader);
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &fileNameStr);	
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);
	
// -------- uri header

		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sURLHeader);
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);

	
// -------- email header

		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEmailHeader);
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);

// -------- connection information header
		
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sConnectionHeader);
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);

// -------- time header

		// t=0 0 is a permanent always available movie (doesn't ever change unless we change the code)
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sPermanentTimeHeader);
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);
		
// -------- control header

		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sStaticControlHeader);
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &sEOL);
...
// -------- sdp data
		...
		int theSDPBodyLen = 0;
		theSDPData.Ptr = theFile->fFile.GetSDPFile(&theSDPBodyLen);
		theSDPData.Len = theSDPBodyLen;
		sdpDataDeleter.SetObject(theSDPData.Ptr); // Just in case we fail, we will clean up. But we clear the deleter if we succeed.
		
		totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &theSDPData);
...
// -------- add buffer delay
		...
		if (sAddClientBufferDelaySecs > 0) // increase the client buffer delay by the preference amount.
		{
			...
			char temp[64];
			::sprintf(temp, "a=x-bufferdelay:%.2f",bufferDelay);
			StrPtrLen bufferDelayStr(temp);	
				
			totalSDPLength += ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, &bufferDelayStr);
		}
...
QTSS_Error DoPlay(QTSS_StandardRTSP_Params* inParamBlock)
{
	...
	else
	{
		...
		if ((theErr != QTSS_NoErr) || (theLen != sizeof(Float64)))
			theStartTimeP = &sDefaultStartTime;
