...
public:
	static void ResolveRelayPrefs(XMLParser* relayPrefs)
	{
		...
		else sDoResolveAgain = true;	// it's already resolving, tell it to try again when it's done
...
void RemoveAllOutputs(RelaySession* inSession)
{
	...
	for (OSQueueIter iter(RelayOutput::GetOutputQueue()); !iter.IsDone(); )
	{
		...
		iter.Next();
                ...
                {	
                    inSession->RemoveOutput(theOutput);
                    delete theOutput;
                }
	}
...
QTSS_Error SetupAnnouncedSessions(QTSS_StandardRTSP_Params* inParams)
{
    ...
    if ( (*theMethod == qtssSetupMethod) && (*theMode == qtssRTPTransportModeReceive) )
    	setup = true;
    else if (*theMethod == qtssPlayMethod)
    	play = true;
    
    // We are only interested in SETUP mode=receive and PLAY requests
    if (!(setup || play))
    	return QTSS_NoErr;
	...
	// If the RTSPSessionID is not in the queue
 	if (theElem == NULL)
 	{
 		// If it's a SETUP mode=receive add the RTSPSessionID to the queue
 		if(setup)
 		{
...
//			continue;
		...
		if ((infoIP == inIP) && (infoURL.Equal(inURL)))
		{
			info = announceInfo;
			break;
		}	
...
void ReadRelayPrefsFile()
{
	...
	DNSResolverThread::ResolveRelayPrefs(xmlFile);
}
