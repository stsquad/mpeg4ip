...
void	QTSSDictionary::SetNumValues(QTSS_AttributeID inAttrID, UInt32 inNumValues)
{
	...
	if (inNumValues >= theAttrs[theMapIndex].fNumAttributes)
		return;
	...
	if (theAttrs[theMapIndex].fIsDynamicDictionary || (dataType == qtssAttrDataTypeCharArray))
	{
		...
		for (UInt32 i = theAttrs[theMapIndex].fNumAttributes - 1; i >= inNumValues; i--)
		{
			this->RemoveValue(inAttrID, i, kDontObeyReadOnly);
		}
	}
	else
	{
