...
QTSS_Error RTPStream::Setup(RTSPRequestInterface* request, QTSS_AddStreamFlags inFlags)
{
	...
	{
		fIsTCP = true;
		fSession->GetOverbufferWindow()->SetWindowSize(-1);
...
void	RTPStream::AppendRTPInfo(QTSS_RTSPHeader inHeader, RTSPRequestInterface* request, UInt32 inFlags, Bool16 lastInfo)
{
	...
	{
		::sprintf(rtpTimeBuf, "%lu", fFirstTimeStamp);
...
//				printf("Q=%d, delay = %qd\n", GetQualityLevel(), inCurrentPacketDelay);
	...
	{
		if ((inCurrentPacketDelay > fAlwaysThinDelay) && (GetQualityLevel() < fNumQualityLevels))
		...
		{
			if (!fWaitOnLevelAdjustment && (GetQualityLevel() < fNumQualityLevels))
			{
				SetQualityLevel(GetQualityLevel() + 1);
				fWaitOnLevelAdjustment = true;
			}
...
//				printf("Packet %d for time %qd sent at %qd (%d bytes)\n", theSeqNum, thePacket->packetTransmitTime - firstTime, theTime - startTime, inLen);
			}
		}	
