...
Bool16 XMLParser::ParseFile(char* errorBuffer, int errorBufferSize)
{
	...
	{
		if (errorBuffer != NULL)
			::sprintf(errorBuffer, "Couldn't read xml file");
		return false;	// we don't have a valid file;
	}
	
	char* fileData = NEW char[fFile.GetLength() + 1];
	UInt32 theLengthRead = 0;
	fFile.Read(0, fileData, fFile.GetLength(), &theLengthRead); 
...
bool XMLTag::ParseTag(StringParser* parser, DTDVerifier* verifier, char* errorBuffer, int errorBufferSize)
{
	while (true)
	{
		...
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "Couldn't find a valid tag");
		...
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "End tag with no begin tag on line %d", parser->GetCurrentLineNumber());
	...
	if (temp.Len == 0)
	{
		if (errorBuffer != NULL)
		{
			if (parser->GetDataRemaining() == 0)
				::sprintf(errorBuffer, "Unexpected end of file on line %d", parser->GetCurrentLineNumber());
			else
				::sprintf(errorBuffer, "Unexpected character (%c) on line %d", parser->PeekFast(), parser->GetCurrentLineNumber());
		}
	...
	while ((parser->PeekFast() != '>') && (parser->PeekFast() != '/'))
	{
		...
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "Missing '=' after attribute %s on line %d", attr->fAttrName, parser->GetCurrentLineNumber());
			return false;	// bad attribute specification
		}
		if (!parser->Expect('"'))
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "Attribute %s value not in quotes on line %d", attr->fAttrName, parser->GetCurrentLineNumber());
			return false;	// bad attribute specification
		}
		...
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "Attribute %s not allowed in tag %s on line %d", attr->fAttrName, fTag, parser->GetCurrentLineNumber());
			return false;	// bad attribute specification
		}
		...
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "Bad value for attribute %s on line %d", attr->fAttrName, parser->GetCurrentLineNumber());
			return false;	// bad attribute specification
		}
	...
	if (parser->PeekFast() == '/')
	{
		...
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "'>' must follow '/' on line %d", parser->GetCurrentLineNumber());
			return false;	// bad attribute specification
		}
	...
	{
		if (errorBuffer != NULL)
			::sprintf(errorBuffer, "Bad format for tag <%s> on line %d", fTag, parser->GetCurrentLineNumber());
		return false;	// bad attribute specification
	}
	...
	while(true)
	{
		...
		{
			if (errorBuffer != NULL)
				::sprintf(errorBuffer, "Reached end of file without end for tag <%s> declared on line %d", fTag, tagStartLine);
			return false;
		}
		if ((*parser)[1] == '/')
		{
				...
				if (tempParser.GetDataRemaining() > 0)
				{
					if (errorBuffer)
					{
						if (fEmbeddedTags.GetLength() > 0)
							::sprintf(errorBuffer, "Unexpected text outside of tag on line %d", tagStartLine);
						else
							::sprintf(errorBuffer, "Tag <%s> on line %d not allowed to have data", fTag, tagStartLine);
					}
				}
			}
		...
		if (((*parser)[1] != '!') && ((*parser)[1] != '?'))
		{
			...
			{
				if (errorBuffer != NULL)
					::sprintf(errorBuffer, "Tag %s not allowed in tag %s on line %d", tag->GetTagName(), fTag, parser->GetCurrentLineNumber());
				return false;	// bad attribute specification
			}
		}
		else
	...
	if (!temp.Equal(fTag))
	{
		...
		if (errorBuffer != NULL)
			::sprintf(errorBuffer, "End tag </%s> on line %d doesn't match tag <%s> declared on line %d", newTag, parser->GetCurrentLineNumber(),fTag, tagStartLine);
		delete newTag;
		return false;	// bad attribute specification
	}
	...
	{
		if (errorBuffer != NULL)
			::sprintf(errorBuffer, "Couldn't find end of tag <%s> declared on line %d", fTag, tagStartLine);
		return false;	// bad attribute specification
	}
...
{
	XMLAttribute* attr = NEW XMLAttribute;
	StrPtrLen temp((char*)attrName);
	attr->fAttrName = temp.GetAsCString();
	temp.Set((char*)attrValue);
	attr->fAttrValue = temp.GetAsCString();
