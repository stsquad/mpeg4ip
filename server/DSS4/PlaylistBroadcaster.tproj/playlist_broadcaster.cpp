...
/* changed by emil@popwire.com (see relaod.txt for info) */
	if(mTimeFile!=NULL)
	{
		...
		{	
			...
			fprintf(fTimeFile,"b=%02d:%02d:%02d:%06d %ld %s", tm_start.tm_hour, tm_start.tm_min, tm_start.tm_sec, start.tv_usec,startTime,timestringStart);
			char *timestringEnd = ::ctime(&endTime);
			fprintf(fTimeFile,"e=%02d:%02d:%02d:%06d %ld %s", tm_end.tm_hour, tm_end.tm_min, tm_end.tm_sec, end.tv_usec,endTime,timestringEnd);
			fprintf(fTimeFile,"d=%02d:%02d:%02d:%06d %d \n", tm_dur.tm_hour, tm_dur.tm_min, tm_dur.tm_sec, dur.tv_usec, dur.tv_sec);
