...
#if __MacOSXServer__
extern "C" {
//Aparently "bool" is defined in some system headers :(
#define bool bool2
#define __private_extern__ extern
#include <mach-o/dyld.h>
#undef bool
#undef __private_extern
}
#elif __MACOS__
	//CFM Includes here
#elif __Win32__
...
{
    printf("dyld found undefined symbol: %s\n",  symbolName);
    exit(0);
}

NSModule multiple_symbol_handler (NSSymbol /*s*/, NSModule /*old*/, NSModule theNew)
{
    printf("dyld found duplicate symbol\n");
    return(theNew);
}
...
{
    printf("dyld errors during link edit for file\n");
    exit(0);
}

#endif /*__MacOSXServer__*/


void OSCodeFragment::Initialize()
{
#if defined(__MacOSXServer__)
...
#elif defined(__MacOSXServer__)
    NSObjectFileImage image;
    if (NSCreateObjectFileImageFromFile(inPath, &image) == NSObjectFileImageSuccess)
    	fFragmentP = NSLinkModule(image, inPath, TRUE);
...
#elif defined(__MacOSXServer__)
    NSUnLinkModule(fFragmentP,FALSE);
...
#elif defined(__MacOSXServer__)
    NSSymbol symbol;
    char *symname2 = (char*)malloc(sizeof(char)*(strlen(inSymbolName)+2));
    sprintf(symname2, "_%s", inSymbolName);
