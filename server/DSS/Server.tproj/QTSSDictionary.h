...
class QTSSDictionaryMap
{
	public:
		...
		Bool16					IsPreemptiveSafe(UInt32 inIndex) 
			{ Assert(inIndex < fNextAvailableID); return fAttrArray[inIndex]->fAttrInfo.fAttrPermission & qtssAttrModePreempSafe; }

		Bool16					IsWriteable(UInt32 inIndex) 
			{ Assert(inIndex < fNextAvailableID); return fAttrArray[inIndex]->fAttrInfo.fAttrPermission & qtssAttrModeWrite; }
		...
		static QTSSDictionaryMap*		GetMap(UInt32 inIndex)
			{ Assert(inIndex < kNumDictionaries); return sDictionaryMaps[inIndex]; }
