...
#if 0
 char* encBuffer = NULL;
 UInt32 encBufOffset = 0;
 if (fEncodedBytesRemaining > 0)
 {
  encBuffer = NEW char[inBufLen];
  ::memcpy(encBuffer, &fRequestBuffer[fCurOffset -
fEncodedBytesRemaining], fEncodedBytesRemaining);
  fCurOffset -= fEncodedBytesRemaining;
  encBufOffset = fEncodedBytesRemaining;
  fEncodedBytesRemaining = 0;
 }

 // If data is encoded (should be true if there were encoded bytes
remaining in the first place)
 if (fDecode)
 {
  ...
  {
   encBuffer = NEW char[inBufLen];
   Assert(encBufOffset == 0);
  }

  // Read the remaining encoded data from the socket into the encBuffer
  // Since lengthRead bytes are already in ioBuffer, try reading the
remaining bytes
  sockErr = fSocket->Read(&encBuffer[encBufOffset], (inBufLen -
lengthRead), outLengthRead);

  encBufOffset += *outLengthRead;
  encBuffer[encBufOffset] = '\0';

  // If there is data to be decoded
  if (encBufOffset > 0)
  {
    // Decode up through the last chunk of 4. The fEncodedBytesRemaining
variable is
    // set so that we can retrieve the remaining bytes in the next read
call.
   fEncodedBytesRemaining = encBufOffset & 3;

   // Let our friendly Base64Decode function know this by NULL
terminating at that point
   UInt32 bytesToDecode = encBufOffset - fEncodedBytesRemaining;
   char endChar = encBuffer[bytesToDecode];
   encBuffer[bytesToDecode] = '\0';
   ...
   while (encodedBytesConsumed < bytesToDecode)
   {
    ...
    UInt32 bytesDecoded = Base64decode(&ioBuffer[lengthRead], encBuffer
+ encodedBytesConsumed);

    // If bytesDecoded is 0, we will end up being in an endless loop.
The
    // base64 must be corrupt, so let's just return an error and abort
    if (bytesDecoded == 0)
     break;

    lengthRead += bytesDecoded;

    // Assuming the stream is valid, the # of encoded bytes we just
consumed is
    // 4/3rds of the number of decoded bytes returned by the decode
function,
   ...
   encBuffer[bytesToDecode] = endChar;

   Assert(lengthRead < inBufLen);
   Assert(encodedBytesConsumed == bytesToDecode);

   // Copy the remaining encoded bytes (< 4) into the fRequestBuffer,
since encBuffer is temporary
   // and we need to retrieve them in the next read call.
   ::memcpy(&fRequestBuffer[fCurOffset], &encBuffer[encBufOffset -
fEncodedBytesRemaining], fEncodedBytesRemaining);
   fCurOffset += fEncodedBytesRemaining;
   // Free the encBuffer now
   encBufOffset = 0;
  }
  if (encBuffer != NULL)
   delete encBuffer;
 }
 else // Data is not encoded
 {
  sockErr = fSocket->Read(&ioBuffer[lengthRead], (inBufLen -
lengthRead), outLengthRead);
  lengthRead += *outLengthRead;
 }

 // Set outLengthRead to the total length read (which is same as
lengthRead)
 *outLengthRead = lengthRead;

 // Set the return error value
 // If the client has closed the connection or there is no message body
 if ((sockErr == ENOTCONN) && (lengthRead == 0))
  err = QTSS_RequestFailed;
 else if (sockErr == OS_NoErr)
  err = QTSS_NoErr;
 else if (sockErr == EAGAIN)
 {
  Assert(lengthRead < inBufLen);
  err = QTSS_WOULDBLOCK;
 }
 else
  err = sockErr; // If error is some other than above, simply return it
