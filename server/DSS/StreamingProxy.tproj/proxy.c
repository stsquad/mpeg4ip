...
#else if defined( _sparc) || defined(HAVE_SETPRIORITY)
	...
	switch (s->state) {
		...
		case stParseClientCommand:
			...
			for (pBuf = s->cinbuf; (p = str_sep(&pBuf, "\r\n")) != NULL; ) {
				...
				if (is_command(p, cmd, temp)) {
					...
					if (make_udp_port_pair(s->server_interface_addr, s->server_ip, &t->RTP_S2P, &t->RTCP_S2P) == -1) {						s->server_skt = INVALID_SOCKET;
		...
		case stServerTransactionRecv:
			...
			if (canRead > 0) 
			{
				...
				{
					//if (gDebug) printf("problems reading from server (%d)", GetLastSocketError(s->server_skt));
			...
			{				
				...
				for (pBuf = s->sinbuf; (p = str_sep(&pBuf, "\r\n")) != NULL; ) 
				{
					...
					// Drop a pointer to where the content body might begin, if this is in fact the last line
			...
			if ((s->totalContentLength > 0) && (s->amtInServerInBuffer < s->totalContentLength))
			{	
				//if (gDebug) printf("2- not enough in buffer content length %li, amt in buffer %li\n", (long)s->totalContentLength, (long)s->amtInServerInBuffer);
				break;
			}
			...
			if (s->contentLength) 
			{
				if (s->transaction_type == ttDescribe) 
				{
					...
					nextBuffPos = get_line_str( lineBuff, nextBuffPos, 1024 );
				...
				int port = atoi(line + pmatch[2].rm_so);
				...
				else
				  {
				    *(line+pmatch[1].rm_eo)='\0';
					...
					#else
				    	if( inet_aton(line+pmatch[1].rm_so,&bindaddr)==0 )
				    #endif
