...
#elif __Win32__
static char*	sDefaultLogDir		 	= "c:\Program Files\Darwin Streaming Server\httpLogs";
#else
...
class QTSSHttpAccessLog : public QTSSRollingLog
{
	public:
		...
		QTSSHttpAccessLog() : QTSSRollingLog() {}
		virtual ~QTSSHttpAccessLog() {}
	
		virtual char* GetLogName() { return QTSSModuleUtils::GetStringPref(sPrefs, "http_logfile_name", sDefaultLogName);}
		virtual char* GetLogDir()  { return QTSSModuleUtils::GetStringPref(sPrefs, "http_logfile_dir", sDefaultLogDir); }
...
#pragma mark __QTSS_HTTP_FILE_MODULE__
...
QTSS_Error RereadPrefs()
{
	...
	QTSSModuleUtils::GetPref(sPrefs, "http_logging", 	qtssAttrDataTypeBool16,
								&sLogEnabled, &sDefaultLogEnabled, sizeof(sLogEnabled));
	QTSSModuleUtils::GetPref(sPrefs, "http_logfile_size", 	qtssAttrDataTypeUInt32,
								&sMaxLogBytes, &sDefaultMaxLogBytes, sizeof(sMaxLogBytes));
	QTSSModuleUtils::GetPref(sPrefs, "http_logfile_interval", 	qtssAttrDataTypeUInt32,
...
QTSS_Error FilterRequest(QTSS_Filter_Params* inParams)
{
	...
	if ( (theStateP == NULL) || (theLen != sizeof(UInt32)) )
	{
		...
		OSCharArrayDeleter httpFolderDtr(QTSSModuleUtils::GetStringPref(sPrefs, "http_folder", ""));
		...
		switch (type) 
		{
			...
			{	
...
#if HTTP_FILE_DEBUGGING
											printf("Number of .mov files in the directory: %lu\n", fileCount);
#endif			
											...
											// We need content length string for logging purposes 
											::sprintf(contentLength, "%lu", moovLen);
			...
			case transferRefMovieFile:		
			{
											...
											::memcpy(theFileBuffer, responseHeader->Ptr, responseHeader->Len);	
		       								...
		       								// We need content length string for logging purposes 
		       								::sprintf(contentLength, "%lu", index);
...
#if HTTP_FILE_DEBUGGING
										printf("Got %lu bytes back from file read. Now at: %"_64BITARG_"u\n", theRecvLen, theOffset);
#endif
...
#if HTTP_FILE_DEBUGGING
	       								printf("Got %lu bytes back from socket write.\n", theWrittenLen);
#endif
...
void LogRequest(QTSS_RTSPSessionObject inRTSPSession)
{
	...
	Bool16 result = QTSSRollingLog::FormatDate(theDateBuffer);         // date field
