...
#pragma mark __RTP_FILE_SESSION_METHODS__
...
RTPFileSession::ErrorCode	RTPFileSession::Seek(Float64 inTime)
{
	...
	SInt64 theBlockLocation = fFile->GetBlockLocation(inTime);
...
#pragma mark __RTP_FILE_METHODS__
...
UInt32		RTPFile::GetTrackBytes(UInt32 inTrackID)
{
	for (UInt32 x = 0; x < fHeader.fNumTracks; x++)
	{
		if (fTrackInfo[x].fID == inTrackID)
