...
static UInt32	sDefaultWorsesToThin 			= 3;
...
QTSS_Error RereadPrefs()
{
	...
	QTSSModuleUtils::GetPref(sPrefs, "loss_thin_tolerance", 	qtssAttrDataTypeUInt32,
								&sLossThinTolerance, &sDefaultLossThinTolerance, sizeof(sLossThinTolerance));
	QTSSModuleUtils::GetPref(sPrefs, "num_losses_to_thin", 	qtssAttrDataTypeUInt32,
								&sNumLossesToThin, 		&sDefaultNumLossesToThin, 	sizeof(sNumLossesToThin));
	QTSSModuleUtils::GetPref(sPrefs, "loss_thick_tolerance", 	qtssAttrDataTypeUInt32,
								&sLossThickTolerance, 	&sDefaultLossThickTolerance, sizeof(sLossThickTolerance));
	QTSSModuleUtils::GetPref(sPrefs, "num_losses_to_thick", 	qtssAttrDataTypeUInt32,
								&sLossesToThick, 		&sDefaultLossesToThick, sizeof(sLossesToThick));
	QTSSModuleUtils::GetPref(sPrefs, "num_worses_to_thin", qtssAttrDataTypeUInt32,
...
QTSS_Error ProcessRTCPPacket(QTSS_RTCPProcess_Params* inParams)
{
	...
	if ((uint16Ptr != NULL) && (theLen == sizeof(UInt16)))
	{
		...
		if (thePercentLoss > sLossThinTolerance)
		{
			...
			{
#if FLOW_CONTROL_DEBUGGING
				printf("Percent loss too high: Incrementing percent loss count to %lu\n", theNumLossesAboveTol);
#endif
		...
		else if (thePercentLoss < sLossThickTolerance)
		{
			...
			{
#if FLOW_CONTROL_DEBUGGING
				printf("Percent is low: Incrementing percent loss count to %lu\n", theNumLossesBelowTol);
#endif
	...
	if ((uint16Ptr != NULL) && (theLen == sizeof(UInt16)))
	{
		...
		if (isGettingWorse != 0)
		{
			...
			{
#if FLOW_CONTROL_DEBUGGING
				printf("Client reporting getting worse. Incrementing num worses count to %lu\n", theNumWorses);
#endif
