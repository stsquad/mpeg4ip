...
{ 	
	...
	if (!fInitialized)
	{
		...
		if (GetFields() == NULL)
			InitializeAllFields(true, NULL, nodeSource);
...
ElementNode::~ElementNode()
{
	...
	//printf("ElementNode::~ElementNode delete %s Element Node # fields = %lu\n",GetNodeName(), fNumFields);
...
void ElementNode::SetFields(UInt32 i, QTSS_Object attrInfoObject)
{
	...
	{
		//printf("QTSS_GetValue err = %ld attrInfoObject=%lu qtssAttrDataType = %lu \n",err, (UInt32)  attrInfoObject, (UInt32) fFieldIDs[i].fAPI_Type);
	}
	...
	//printf("ElementNode::SetFields name=%s api_id=%ld \n",fFieldIDs[i].fFieldName, fFieldIDs[i].fAPI_ID);
...
ElementNode* ElementNode::CreateArrayAttributeNode(UInt32 index, QTSS_Object source, QTSS_Object attributeInfo, UInt32 arraySize )
{
	...
	//printf("ElementNode::CreateArrayAttributeNode name = %s index = %lu arraySize =%lu \n",fFieldIDs[index].fFieldName, index,arraySize);
	...
	{	//printf("ElementNode::CreateArrayAttributeNode index = %lu this->GetNodeInfoPtr(index) == NULL \n",index);
	}
	...
	{	
		...
		{	fieldPtr->fFieldType = eNode;
			...
			Warn(err == QTSS_NoErr);
			if (err != QTSS_NoErr)
			{	//printf("Error Getting Value for %s type = qtssAttrDataTypeQTSS_Object err = %lu\n", fieldPtr->fFieldName,err);
...
void ElementNode::InitializeAllFields(Bool16 allocateFields, QTSS_Object defaultAttributeInfo, QTSS_Object source )
{
	...
	{			
		...
		//printf("ElementNode::InitializeAllFields AllocateFields numFields =  %lu error = %ld \n",numFields, err);
	}
	...
	{	
		...
		{	
			...
			{	err = QTSS_GetAttrInfoByIndex(source, i, &theAttributeInfo);
				Assert(err == QTSS_NoErr);			
				if (err != QTSS_NoErr)
				{	//printf("QTSS_GetAttrInfoByIndex returned err = %lu \n",err);
				}
			}
			else
			...
			{
				//printf("ElementNode::InitializeAllFields field index = %lu name = %s api Source = %lu \n", i,fFieldIDs[i].fFieldName, (UInt32) source);
			}
			
			err = fElementMap->Register(GetOSRef(i));
			if (err != QTSS_NoErr)
			{	//printf("ElementNode::InitializeAllFields  Register returned err = %lu field = %s node=%s \n",err,GetName(i),GetMyName());
			}
			Assert(err == QTSS_NoErr);	
		}
	}
...
void ElementNode::SetUpSingleNode(QueryURI *queryPtr,  StrPtrLen *currentSegmentPtr, StrPtrLen *nextSegmentPtr, SInt32 index, QTSS_Initialize_Params *initParams) 
{
	...
	if (queryPtr && currentSegmentPtr && nextSegmentPtr&& initParams) do
	{
		...
		if (NULL == theNodePtr) 
		{
			//printf(" ElementNode::SetUpSingleNode (NULL == theNodePtr(%ld)) name=%s \n",index,GetName(index));
			break;
		}
		...
		{	StrPtrLen nodeNameSPL(nodeName);
			if	(	(!nodeNameSPL.Equal(*nextSegmentPtr) && !nextSegmentPtr->Equal(sDoAllSPL))
					&&
		...
		if (nodePtr != NULL)
		{
			...
			if (!nodePtr->fInitialized)
			{
				...
				//printf("ElementNode::SetUpSingleNode GetValue source = %lu name = %s id = %lu \n",(UInt32)  GetSource(),(UInt32)  GetName(index),(UInt32) GetAPI_ID(index));
...
Bool16 ElementNode::SetUpOneDataField( UInt32 index)
{
	...
	else
	{
		...
		//printf("ElementNode::SetUpOneDataField %s IsNodeElement index = %lu\n",GetName(index),(UInt32)  index);	
...
void ElementNode::SetUpSingleElement(QueryURI *queryPtr, StrPtrLen *currentSegmentPtr,StrPtrLen *nextSegmentPtr, SInt32 index, QTSS_Initialize_Params *initParams) 
{
	...
	if 	(	(queryPtr->RecurseParam() && (nextSegmentPtr->Len == 0))
			|| 
			(indexNodeNameSPL.Equal(*nextSegmentPtr) || nextSegmentPtr->Equal(sDoAllSPL)) 
...
void ElementNode::SetUpAllNodes(QueryURI *queryPtr, StrPtrLen *currentSegmentPtr, StrPtrLen *nextSegmentPtr, QTSS_Initialize_Params *initParams) 
{
	...
	for(SInt32 index = 0; !IsStopItem(index); index ++)
	{
		...
		//printf("ElementNode::SetUpAllNodes index = %ld nextSegmentPtr=", index);PRINT_STR(nextSegmentPtr);
		StrPtrLen indexNodeNameSPL;
		GetNameSPL(index,&indexNodeNameSPL);
		if 	(	IsNodeElement(index) 
				&&
		...
		{
			//printf("ElementNode::SetUpAllNodes skip index = %ld indexNodeName=", index);PRINT_STR(&indexNodeNameSPL);
			//printf("ElementNode::SetUpAllNodes skip nextSegmentPtr=");PRINT_STR(nextSegmentPtr);
		}
	}
}
...
UInt32 ElementNode::CountAttributes(QTSS_Object source)
{
	...
	//printf("ElementNode::CountAttributes SOURCE = %lu \n", (UInt32) source);

	UInt32 numFields = 0;

	(void) QTSS_GetNumAttributes (source, &numFields);
	
	//printf("ElementNode::CountAttributes %s = %lu \n",GetNodeName() ,numFields);
...
UInt32 ElementNode::CountValues(QTSS_Object source, UInt32 apiID)
{
	...
	//printf("ElementNode::CountValues %s = %lu \n",GetNodeName() ,numFields);
...
void ElementNode::RespondWithSelfAdd(QTSS_StreamRef inStream, QueryURI *queryPtr)
{
	...
	//printf("ElementNode::RespondWithSelfAdd NODE = %s index = %ld \n",GetNodeName(), (SInt32) index);
...
*/
	...
	//printf("QTSS_AddInstanceAttribute(source=%lu, name=%s, NULL, %d, %lu)\n",GetSource(),nameDeleter.GetObject(),attrDataType,accessFlags);
	if (err) 
	{	UInt32 result = 400;
		if (err == QTSS_AttrNameExists)
		{	sprintf(messageBuffer,  "The name %s already exists QTSS_Error=%ld from QTSS_AddInstanceAttribute",nameDeleter.GetObject(),err);
		}
		else
		{	sprintf(messageBuffer,  "QTSS_Error=%ld from QTSS_AddInstanceAttribute",err);
		}
		(void) queryPtr->EvalQuery(&result, messageBuffer);
		return;
	}
	...
	if (err) 
	{	UInt32 result = 400;
		sprintf(messageBuffer,  "QTSS_Error=%ld from QTSS_GetAttrInfoByName",err);
		(void) queryPtr->EvalQuery(&result, messageBuffer);
		return;
	}
	...
	if (err) 
	{	UInt32 result = 400;
		sprintf(messageBuffer,  "QTSS_Error=%ld from QTSS_GetValue",err);
		(void) queryPtr->EvalQuery(&result, messageBuffer);
		return;
	}
	...
	if (err) 
	{	UInt32 result = 400;
		sprintf(messageBuffer,  "QTSS_Error=%ld from QTSS_SetValue",err);
		(void) queryPtr->EvalQuery(&result, messageBuffer);
		return;
	}
...
void ElementNode::RespondWithSelf(QTSS_StreamRef inStream, QueryURI *queryPtr)
{
	...
	{	RespondToDel(inStream, GetMyKey(),queryPtr,true);
		return;	
	}
...
void 	ElementNode::RespondToAdd(QTSS_StreamRef inStream, SInt32 index,QueryURI *queryPtr)
{
	...
	//printf("ElementNode::RespondToAdd NODE = %s index = %ld \n",GetNodeName(), (SInt32) index);
	if (GetNumFields() == 0) 
	{
		UInt32 result = 405;
	...
	{	
		...
		if (attributeLen != len)
		{	UInt32 result = 400;
			sprintf(messageBuffer,  "Data length %lu does not match attribute len %lu",len, attributeLen);
			(void) queryPtr->EvalQuery(&result, messageBuffer);
			return;
		}
		...
		if (err) 
		{	UInt32 result = 400;
			sprintf(messageBuffer,  "QTSS_Error=%ld from QTSS_GetNumValues",err);
			(void) queryPtr->EvalQuery(&result, messageBuffer);
			return;
		}
		
		//printf("ElementNode::RespondToAdd QTSS_SetValue object=%lu attrID=%lu, index = %lu valuePtr=%lu valuelen =%lu \n",GetSource(),GetAPI_ID(index), GetAttributeIndex(index), valueBuff,len);
...
void 	ElementNode::RespondToSet(QTSS_StreamRef inStream, SInt32 index,QueryURI *queryPtr)
{
	...
	//printf("ElementNode::RespondToSet NODE = %s index = %ld \n",GetNodeName(), (SInt32) index);
	...
	{	
		...
		//printf("ElementNode::RespondToSet QTSS_SetValue object=%lu attrID=%lu, index = %lu valuePtr=%lu valuelen =%lu \n",GetSource(),GetAPI_ID(index), GetAttributeIndex(index), valueBuff,len);
	...
	{	UInt32 result = 400;
		(void) queryPtr->EvalQuery(&result, messageBuffer);
		//printf("ElementNode::RespondToSet %s len = %lu ",messageBuffer, result);
		return;
	}
...
void 	ElementNode::RespondToDel(QTSS_StreamRef inStream, SInt32 index,QueryURI *queryPtr,Bool16 delAttribute)
{
	...
	//printf("ElementNode::RespondToDel NODE = %s index = %ld \n",GetNodeName(), (SInt32) index);
	...
	{	
		...
		sprintf(messageBuffer,  "QTSS_Error=%ld from QTSS_RemoveValue", err);
		//printf("ElementNode::RespondToDel QTSS_RemoveValue object=%lu attrID=%lu index=%lu %s\n",GetSource(),GetAPI_ID(index),GetAttributeIndex(index),messageBuffer);
		if (err) 
		{	(void) queryPtr->EvalQuery(&result, messageBuffer);
		}	
	}
	...
	{	
		//printf("ElementNode::RespondToDel QTSS_RemoveInstanceAttribute object=%lu attrID=%lu \n",GetSource(),GetAPI_ID(index));
		err = QTSS_RemoveInstanceAttribute(GetSource(),GetAPI_ID(index));		
		if (err) 
		{
			sprintf(messageBuffer,  "QTSS_Error=%ld from QTSS_RemoveInstanceAttribute",err);
		}	
	...
	{	UInt32 result = 400;
		(void) queryPtr->EvalQuery(&result, messageBuffer);
		//printf("ElementNode::RespondToDel %s len = %lu ",messageBuffer, result);
		return;
	}
...
void ElementNode::RespondToGet(QTSS_StreamRef inStream, SInt32 index,QueryURI *queryPtr)
{
	...
	//printf("ElementNode::RespondToGet NODE = %s index = %ld \n",GetNodeName(), (SInt32) index);
	...
	//printf("ElementNode::RespondToGet QTSS_SetValue object=%lu attrID=%lu, index = %lu \n",GetSource(),GetAPI_ID(index), GetAttributeIndex(index));
	...
	//printf("ElementNode::RespondToGet %s:len = %lu",GetName(index),(UInt32) GetNameLen(index));
...
void 	ElementNode::RespondToKey(QTSS_StreamRef inStream, SInt32 index,QueryURI *queryPtr)
{
	...
	//printf("ElementNode::RespondToKey command = %ld node =%s index=%ld\n",command, GetNodeName(),index);
...
void ElementNode::RespondWithSingleElement(QTSS_StreamRef inStream, QueryURI *queryPtr, StrPtrLen *currentSegmentPtr) 
{
	...
	//printf("ElementNode::RespondWithSingleElement key = %ld\n",key);
	//printf("currentSegmentPtr="); PRINT_STR(currentSegmentPtr);
	
	if (key < 0) 
	{
		//printf("ElementNode::RespondWithSingleElement key = %ld NOT FOUND no ELEMENT\n",key);
		return;
	}
...
void ElementNode::RespondWithAllElements(QTSS_StreamRef inStream, QueryURI *queryPtr, StrPtrLen *currentSegmentPtr)
{
	...
	for ( index = 0; !IsStopItem(index);index++)
	{
		...
		{	
			...
			{
				//printf("ElementNode::RespondWithAllElements Current Node index= %lu NULL = %s\n",index,GetName(index));
...
void ElementNode::RespondToQuery(QTSS_StreamRef inStream, QueryURI *queryPtr,StrPtrLen *currentPathPtr)
{
	...
	{	
		...
		doAllNext = nextSegment.Equal(sDoAllSPL);		
		doAllNextNext = nextSegment.Equal(sDoAllSPL);
...
void ElementNode::SetupNodes(QueryURI *queryPtr,StrPtrLen *currentPathPtr,QTSS_Initialize_Params *initParams)
{
	...
	{	
		...
		( void)queryPtr->NextSegment(currentPathPtr, &nextSegment);
		doAll = nextSegment.Equal(sDoAllSPL);
		
		StrPtrLen *thisNamePtr = GetNodeNameSPL();
		//printf("ElementNode::SetupNodes thisNamePtr="); PRINT_STR(thisNamePtr);

		if (	( (doAll) && (currentPathPtr->Equal(*thisNamePtr) || currentPathPtr->Equal(sDoAllSPL)) )
		...
		{ 	//printf("ElementNode::SetupNodes call SetUpSingleElement index=%lu nextSegment=");PRINT_STR( &nextSegment);
