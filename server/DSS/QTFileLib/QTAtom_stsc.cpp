 ...
 */
// $Id: QTAtom_stsc.cpp,v 1.1 2001/02/27 00:56:49 cahighlander Exp $
//
// QTAtom_stsc:
...
//	printf("GetChunkFirstLastSample chunk = %d STCB->chunkNumber_GetChunkFirstLastSample= %d \n",chunkNumber,STCB->chunkNumber_GetChunkFirstLastSample);
	 

	if( STCB == NULL )
	{
//		printf(" QTAtom_stsc::GetChunkFirstLastSample (NULL == STCB) \n");
		STCB = &tempSTCB;
		//STCB->Reset();
	}	 
	...
	{
//		printf("GetChunkFirstLastSample cache hit chunk = %d\n",chunkNumber);
	...
	{
//		printf(" QTAtom_stsc::GetChunkFirstLastSample missed Cache Loop \n");
//		 STCB = &tempSTCB;
	...
	{
//		printf("GetChunkFirstLastSample cached loop chunk start = %d look for chunk = %ld\n",STCB->fLastFirstChunk_GetChunkFirstLastSample,chunkNumber);
	...
	{	
		...
		prevSamplesPerChunk = ((chunkNumber -1 ) * samplesPerChunk) + 1;
...
//	printf("Get Chunk %ld First %ld Last %ld prevSamplesPerChunk %ld samplesPerChunk %ld\n",chunkNumber,*firstSample,*lastSample,prevSamplesPerChunk,samplesPerChunk);
...
Bool16 QTAtom_stsc::SampleToChunkInfo(UInt32 SampleNumber,  UInt32 *samplesPerChunk,  UInt32 *ChunkNumber, UInt32 *SampleDescriptionIndex, UInt32 *SampleOffsetInChunk, QTAtom_stsc_SampleTableControlBlock * STCB) 
{
	...
	if (STCB == NULL ) 
	{
//		printf(" QTAtom_stsc::SampleToChunkInfo (NULL == STCB) \n");
		STCB = &tempSTCB;
	}
...
/*
	...
	{
		printf("QTAtom_stsc::SampleToChunkInfo #calls = %ld, cache hits = %ld \n",gstscCallCount, gstscCacheCount);
		gstscCacheCount = 0;
		gstscCallCount = 0;
...
//	printf("------ QTAtom_stsc::SampleToChunkInfo SampleNumber = %ld\n",SampleNumber);

	if (STCB->fFirstSampleNumber_SampleToChunkInfo == SampleNumber)
	{
//		printf("------ QTAtom_stsc::SampleToChunkInfo cache hit SampleNumber = %ld\n",SampleNumber);
...
//	printf("QTAtom_stsc::SampleToChunkInfo missed cache SampleNumber = %ld\n",SampleNumber);
	...
	{	
		missedCache = true;
//		printf("missed loop Cache!! STCB = %ld STCB->fCurSample_SampleToChunkInfo = %ld  > SampleNumber = %ld \n",STCB, STCB->fCurSample_SampleToChunkInfo, SampleNumber);
...
//			printf("GetSample Info in for loop returning offset %ld \n",aSampleOffsetInChunk);
			return true;
		}	
...
//	printf("GetSample Info fall out of loop returning offset%ld \n",aSampleOffsetInChunk);
	//
