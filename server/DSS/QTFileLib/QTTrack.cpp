 ...
 */
// $Id: QTTrack.cpp,v 1.1 2001/02/27 00:56:49 cahighlander Exp $
//
// QTTrack:
...
QTTrack::ErrorCode QTTrack::Initialize(void)
{
	...
	if( fFile->FindTOCEntry(":udta:name", &tempTOCEntry, &fTOCEntry) ) {
		fTrackName = NEW char[tempTOCEntry->AtomDataLength + 1];
		if( fTrackName != NULL )
	...
	fChunkOffsetAtom = NEW QTAtom_stco(fFile, tempTOCEntry, fDebug, fDeepDebug);
	if( fChunkOffsetAtom == NULL )
		return errInternalError;
...
//	printf("GetSampleInfo QTTrack SampleNumber = %ld \n", SampleNumber);

	if (STCB->fGetSampleInfo_SampleNumber == SampleNumber && STCB->fGetSampleInfo_Length > 0)
	{
//		printf("----- GetSampleInfo Cache Hit QTTrack SampleNumber = %ld \n", SampleNumber);
	...
	else
	{
		...
		STCB->fGetSampleInfo_LastChunkOffset = ChunkOffset;
...
Bool16 QTTrack::GetSizeOfSamplesInChunk(UInt32 chunkNumber, UInt32 * const sizePtr, UInt32 * const firstSampleNumPtr, UInt32 * const lastSampleNumPtr, QTAtom_stsc_SampleTableControlBlock * stcbPtr)
{
	...
	{
//		printf("QTTrack::GetSizeOfSamplesInChunk cache hit %ld \n", chunkNumber);
