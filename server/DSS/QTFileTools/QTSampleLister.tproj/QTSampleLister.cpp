...
/* SIOUX setup */
	...
	if( !DumpHTML ) {
		time_t		unixCreationTime = Track->GetCreationTime() + QT_TIME_TO_LOCAL_TIME;
	...
	UInt32 curSample = 1;
	while( Track->GetSampleInfo(curSample, &Size, &Offset) ) {
		//
		// Get some additional information about this sample.
		Track->GetSampleMediaTime(curSample, &MediaTime);
		...
		printf("<FONT COLOR=black>%010"_64BITARG_"u: track=%02lu; size=%lu</FONT><BR>\n",Offset, Track->GetTrackID(), Size);
				...
				printf("<FONT COLOR=black>%010llu: track=%02lu; size=%lu</FONT><BR>\n",
				   Offset, Track->GetTrackID(), Size);
			#else
				printf("<FONT COLOR=black>%010qu: track=%02lu; size=%lu</FONT><BR>\n",
				   Offset, Track->GetTrackID(), Size);
			#endif
		#else
			printf("<FONT COLOR=black>%010lu: track=%02lu; size=%lu</FONT><BR>\n",
				   (UInt32)Offset, Track->GetTrackID(), Size);
		#endif
#endif
		} else {
		...
		#if HAVE_64BIT_PRINTF
			#if HAVE_64BIT_PRINTF_AS_LL
				printf("%8lu  -  %10lu  %10llu  %10lu\n", curSample, MediaTime, Offset, Size);
			#else
				printf("%8lu  -  %10lu  %10qu  %10lu\n", curSample, MediaTime, Offset, Size);
			#endif
		#else
			printf("%8lu  -  %10lu  %10lu  %10lu\n", curSample, MediaTime, (UInt32)Offset, Size);
		#endif
#endif
		}
